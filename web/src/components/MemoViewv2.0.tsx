...
import { Modal, ModalDialog, Typography, Button } from '@mui/joy'; // 导入 Material-UI 的对话框组件
...
const MemoView: React.FC<Props> = (props: Props) => {
    ...
    const [open, setOpen] = useState(false); // 控制对话框的显示和隐藏
    const [summary, setSummary] = useState(''); // 存储 AI 总结的内容

    const handleAISummaryClick = () => {
        // 这里调用 AI 总结的 API，并将结果设置到 summary 状态中
        // 示例代码：
        // fetch('/api/summarize', {
        //   method: 'POST',
        ...
        // });
        // 示例代码，直接设置总结内容
        setSummary("This is a AI summary");
        setOpen(true);
    };
                            ...
                            <div className="w-auto invisible group-hover:visible flex flex-row justify-between items-center gap-2">
                                ...
                                <Tooltip title="AI 总结" placement="top">
                                    <span className="cursor-pointer flex justify-center items-center hover:opacity-70">
                                        <Bot className="w-4 h-4 text-gray-500 dark:text-gray-400" onClick={handleAISummaryClick} />
                                    </span>
                                </Tooltip>
                            </div>
